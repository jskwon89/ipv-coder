...
(VALUES
  ...
  ('seed-qna-8', '종속변수가 예/아니오인데 회귀분석을 해도 되나요?', '종속변수가 재구매 여부처럼 예/아니오로 나뉩니다. 독립변수는 만족도 점수와 이용 빈도인데 이런 경우에도 일반 회귀분석을 쓰면 되는지 궁금합니다.', 'beomsu.han@gmail.com', 33, '2026-05-01T15:28:00+09:00'::timestamptz),
...
FROM primer_author,
(VALUES
  ('seed-qna-1-answer', 'seed-qna-1', '먼저 보려는 게 차이인지, 관계인지, 영향인지부터 나누면 됩니다. 집단 간 평균 차이를 보려면 t검정이나 ANOVA, 변수들이 같이 움직이는지 보려면 상관분석, 어떤 독립변수가 종속변수에 영향을 주는지 보려면 회귀분석을 많이 씁니다. 질문을 이렇게 나누면 분석 방향이 훨씬 선명해집니다.', '2026-01-01T00:00:30Z'::timestamptz),
  ('seed-qna-2-answer', 'seed-qna-2', '종속변수가 점수나 척도처럼 숫자라면 회귀분석이 잘 맞는 경우가 많습니다. 예를 들어 만족도, 우울 점수, 스트레스 점수처럼 연속형 종속변수를 설명하고 싶을 때 사용합니다. 다만 종속변수가 예/아니오처럼 둘로 나뉘면 일반 회귀가 아니라 로지스틱 회귀를 생각해야 합니다.', '2026-01-01T00:01:30Z'::timestamptz),
  ...
  ('seed-qna-4-answer', 'seed-qna-4', '통제변수는 종속변수에 영향을 줄 수 있는 다른 요인을 잠시 잡아두는 역할을 합니다. 예를 들어 교육 프로그램 효과를 보는데 나이나 기존 점수가 종속변수에 영향을 줄 수 있다면, 그 변수들을 함께 넣어야 핵심 독립변수의 효과를 더 공정하게 볼 수 있습니다. 회귀분석에서 자주 쓰는 방식입니다.', '2026-01-01T00:03:30Z'::timestamptz),
  ...
  ('seed-qna-8-answer', 'seed-qna-8', '종속변수가 예/아니오, 발생/미발생, 합격/불합격처럼 둘로 나뉘면 일반 선형회귀보다 로지스틱 회귀분석을 사용합니다. 결과는 보통 오즈비로 해석합니다. 오즈비가 1보다 크면 발생 가능성이 높아지는 방향, 1보다 작으면 낮아지는 방향으로 이해할 수 있습니다.', '2026-01-01T00:07:30Z'::timestamptz),
