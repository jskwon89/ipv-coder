...
- 헤더 로그인 영역을 메인 헤더(teal 버튼)에서 상단 유틸리티 바 Q&A 우측으로 이동
...
- 비로그인 시: 유틸 바에 `로그인 / 회원가입` 텍스트 링크 표시
- 로그인 시: 유틸 바에 사용자 아이콘 + 이메일 + 로그아웃 표시
- 메인 헤더 우측의 기존 로그인/사용자 영역 제거 (`내 의뢰` 버튼은 유지)

검증:

- 코드 수정만 진행. 로컬 lint/build 미실행.
...
- 모바일 메뉴(`mobileMenuOpen`) 안의 로그인/로그아웃 영역은 그대로 유지함. 데스크톱(lg) 한정 변경.
...
- Q&A seed 10건을 일반 사용자가 실제로 물어보는 듯한 쉬운 질문형 문구로 다시 조정했다.
- 비용, 운영, 사용법 안내 문항은 넣지 않고 통계 방법·분석 선택·해석 기준 중심으로만 유지했다.
- 라이브 Supabase `board_posts`의 `seed-qna-1`~`seed-qna-10`도 같은 문구로 갱신했다.

검증:

- 라이브 Supabase upsert 10건 성공
- 프로덕션 `/api/board/posts?category=qna&limit=10&offset=0` 200 응답 및 새 자연어 제목 10건 확인
...
- Q&A seed는 통계 분석 질문만 유지한다. 견적, 기간, 할인, 서비스 이용법 같은 운영성 문항은 넣지 않는다.
