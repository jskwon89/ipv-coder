        ...
        {VARIABLE_GROUPS.map((group) => {
          ...
          return (
            <div key={group.id} className="border border-border rounded-lg overflow-hidden">
              {/* Group header */}
              <div className="flex items-center gap-2 px-3 py-2 bg-secondary/50 hover:bg-secondary transition-colors">
                <input
                  type="checkbox"
                  checked={allSelected}
                  ref={(el) => {
              ...
              {isExpanded && (
                <div className="px-3 py-2 space-y-1 max-h-[300px] overflow-y-auto">
                  {group.variables.map((v) => (
                    <label
                      key={v.id}
                      className="flex items-start gap-2 py-1 text-sm cursor-pointer hover:bg-secondary/30 rounded px-1 -mx-1"
                    >
                      <input
                        type="checkbox"
