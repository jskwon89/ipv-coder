...
export default function QualResultsPage() {
  const [requests, setRequests] = useState<QualAnalysisRequest[]>([]);
  ...
  const fetchMessages = useCallback(async (reqId: string) => {
    try {
      const res = await fetch(`/api/qual-analysis/${reqId}/messages`);
      const data = await res.json();
  ...
  const handleSendMessage = async () => {
    ...
    setSendingChat(true);
    try {
      await fetch(`/api/qual-analysis/${selectedId}/messages`, {
        method: "POST",
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto">
      ...
      <h1 className="text-2xl font-bold mb-2">질적분석 결과 확인</h1>
      <p className="text-gray-600 text-sm mb-8">의뢰하신 질적분석의 진행 상황과 결과를 확인합니다.</p>
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm">
        ...
        {sorted.length === 0 ? (
          <div className="px-6 py-16 text-center">
            ...
            <Link href="/qual-analysis" className="inline-flex items-center gap-2 px-4 py-2 bg-teal-500 text-white rounded-lg text-sm font-medium hover:bg-teal-600 transition-colors">
              질적분석 의뢰하기
            </Link>
          </div>
        ) : (
        ...
        <>
          ...
          <div className="fixed inset-y-0 right-0 w-full max-w-lg bg-white shadow-xl z-50 flex flex-col animate-slide-in">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 shrink-0">
              <div className="min-w-0 flex-1">
                <h3 className="font-semibold text-gray-900 truncate">{selectedRequest.analysisType || "질적분석"}</h3>
                <div className="flex items-center gap-3 mt-1">
            ...
            <div className="flex-1 overflow-y-auto" ref={chatContainerRef}>
              <div className="px-6 py-4 border-b border-gray-100 space-y-3">
                ...
                {selectedRequest.analysisGoal && (
                  <div><p className="text-xs text-gray-400">분석 목표</p><p className="text-sm text-gray-700 leading-relaxed whitespace-pre-wrap">{selectedRequest.analysisGoal}</p></div>
                )}
