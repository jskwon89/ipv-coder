...
export default function TextResultsPage() {
  const [requests, setRequests] = useState<TextAnalysisRequest[]>([]);
  ...
  const fetchMessages = useCallback(async (reqId: string) => {
    try {
      const res = await fetch(`/api/text-analysis-request/${reqId}/messages`);
      const data = await res.json();
  ...
  const handleSendMessage = async () => {
    ...
    setSendingChat(true);
    try {
      await fetch(`/api/text-analysis-request/${selectedId}/messages`, {
        method: "POST",
  ...
  const parseAnalysisTypes = (json: string) => {
    try { return JSON.parse(json).join(", "); } catch { return json; }
  };

  return (
    <div className="p-8 max-w-5xl mx-auto">
      <ServiceTabs tabs={[{ label: "분석 의뢰", href: "/text-analysis" }, { label: "결과 확인", href: "/text-results" }]} />

      <h1 className="text-2xl font-bold mb-2">텍스트 분석 결과 확인</h1>
      <p className="text-gray-600 text-sm mb-8">의뢰하신 텍스트 분석의 진행 상황과 결과를 확인합니다.</p>
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm">
        ...
        {sorted.length === 0 ? (
          <div className="px-6 py-16 text-center">
            ...
            <Link href="/text-analysis" className="inline-flex items-center gap-2 px-4 py-2 bg-primary text-white rounded-lg text-sm font-medium hover:bg-teal-700 transition-colors">
              텍스트 분석 의뢰하기
            </Link>
          </div>
        ) : (
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-3">
                      <span className="text-sm font-medium text-gray-900 truncate">
                        {parseAnalysisTypes(req.analysisTypes) || "텍스트 분석"}
                      </span>
                    ...
                    <div className="flex items-center gap-3 mt-1">
                      ...
                      {req.dataInputMethod && <span className="text-xs text-gray-400">{req.dataInputMethod}</span>}
        ...
        <>
          ...
          <div className="fixed inset-y-0 right-0 w-full max-w-lg bg-white shadow-xl z-50 flex flex-col animate-slide-in">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 shrink-0">
              <div className="min-w-0 flex-1">
                <h3 className="font-semibold text-gray-900 truncate">{parseAnalysisTypes(selectedRequest.analysisTypes) || "텍스트 분석"}</h3>
                <div className="flex items-center gap-3 mt-1">
              ...
              <div className="px-6 py-4 border-b border-gray-100 space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <p className="text-sm text-gray-700">{parseAnalysisTypes(selectedRequest.analysisTypes)}</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-400">데이터 입력 방식</p>
                    <p className="text-sm text-gray-700">{selectedRequest.dataInputMethod || "-"}</p>
                  </div>
                  <div>
                ...
                {selectedRequest.textContent && (
                  <div><p className="text-xs text-gray-400">텍스트 내용</p><p className="text-sm text-gray-700 leading-relaxed whitespace-pre-wrap line-clamp-6">{selectedRequest.textContent}</p></div>
                )}
