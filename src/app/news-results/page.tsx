...
interface NewsCollectionRequest {
  id: string;
  email: string;
...
function getSummaryLabel(req: NewsCollectionRequest): string {
  if (req.searchType === "keyword") {
    const kws = parseJsonArray(req.keywords);
    return kws.length > 0 ? kws.join(", ") : "키워드 검색";
  }
  return req.keywords
    ? req.keywords.length > 40
      ? req.keywords.slice(0, 40) + "..."
      : req.keywords
    : "문장 검색";
}

export default function NewsResultsPage() {
  const [requests, setRequests] = useState<NewsCollectionRequest[]>([]);
  ...
  const fetchMessages = useCallback(async (reqId: string) => {
    try {
      const res = await fetch(`/api/news-collection/${reqId}/messages`);
      const data = await res.json();
  ...
  const handleSendMessage = async () => {
    ...
    setSendingChat(true);
    try {
      await fetch(`/api/news-collection/${selectedId}/messages`, {
        method: "POST",
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto">
      ...
      <h1 className="text-2xl font-bold mb-2">뉴스 수집 결과 확인</h1>
      <p className="text-gray-600 text-sm mb-8">의뢰하신 뉴스/언론 보도 수집의 진행 상황과 결과를 확인합니다.</p>
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm">
        ...
        {sorted.length === 0 ? (
          <div className="px-6 py-16 text-center">
            ...
            <Link
              href="/news-search"
              className="inline-flex items-center gap-2 px-4 py-2 bg-primary text-white rounded-lg text-sm font-medium hover:bg-teal-700 transition-colors"
            >
              뉴스 수집 의뢰하기
            </Link>
          </div>
      ...
      <div className="mt-8 bg-white rounded-xl border border-gray-200 shadow-sm p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
          <div className="text-center">
            <div className="w-10 h-10 rounded-full bg-primary text-white flex items-center justify-center mx-auto mb-2 text-sm font-bold">1</div>
            <p className="text-sm font-medium">의뢰 접수</p>
            <p className="text-xs text-gray-600 mt-1">검색 조건 입력 및 제출</p>
          </div>
          <div className="text-center">
            <div className="w-10 h-10 rounded-full bg-primary text-white flex items-center justify-center mx-auto mb-2 text-sm font-bold">2</div>
            <p className="text-sm font-medium">검토 안내</p>
            <p className="text-xs text-gray-600 mt-1">검토 후 수집 범위 및 일정 안내</p>
          </div>
          <div className="text-center">
            <div className="w-10 h-10 rounded-full bg-primary text-white flex items-center justify-center mx-auto mb-2 text-sm font-bold">3</div>
            <p className="text-sm font-medium">수집 진행</p>
            <p className="text-xs text-gray-600 mt-1">뉴스/언론 보도 수집</p>
          </div>
          <div className="text-center">
            <div className="w-10 h-10 rounded-full bg-primary text-white flex items-center justify-center mx-auto mb-2 text-sm font-bold">4</div>
            <p className="text-sm font-medium">결과 전달</p>
            <p className="text-xs text-gray-600 mt-1">수집 결과 파일 전달</p>
          </div>
        </div>
      </div>
          ...
          <div className="fixed inset-y-0 right-0 w-full max-w-lg bg-white shadow-xl z-50 flex flex-col animate-slide-in">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 shrink-0">
              <div className="min-w-0 flex-1">
                ...
                <div className="flex items-center gap-3 mt-1">
                  ...
                  <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-purple-50 text-purple-600">
                    {selectedRequest.searchType === "keyword" ? "키워드 검색" : "문장 검색"}
                  </span>
                  <span className="text-xs text-gray-400">
            ...
            <div className="flex-1 overflow-y-auto" ref={chatContainerRef}>
              ...
              <div className="px-6 py-4 border-b border-gray-100 space-y-3">
                ...
                {selectedRequest.searchType === "keyword" ? (
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5 mt-1">
                      {parseJsonArray(selectedRequest.keywords).map((kw) => (
                        <span
                          key={kw}
                          className="inline-flex items-center bg-amber-50 text-primary px-2.5 py-1 rounded-full text-xs font-medium border border-teal-600/20"
                        >
                          {kw}
                        </span>
                      ))}
                  ...
                  <div>
                    <p className="text-xs text-gray-400">검색 문장</p>
                    <p className="text-sm text-gray-700 leading-relaxed mt-1">{selectedRequest.keywords}</p>
                  </div>
                )}
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    <p className="text-xs text-gray-400">수집 목적</p>
                    <p className="text-sm text-gray-700">
                      {purposeLabels[selectedRequest.purpose] || selectedRequest.purpose}
                    </p>
                  </div>
                  <div>
                      ...
                      >
                        <div
                          className={`max-w-[80%] rounded-xl px-4 py-2.5 ${
                            msg.sender === "user"
                              ? "bg-primary/10 border border-teal-600/20 text-gray-900"
