...
import { getContestSearchMessages, addContestSearchMessage } from '@/lib/db';
...
export async function GET(
  ...
  try {
    ...
    const messages = await getContestSearchMessages(id);
    messages.sort(
...
export async function POST(
  ...
  try {
    ...
    const msg = await addContestSearchMessage(id, sender, message.trim());
