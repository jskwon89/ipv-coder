...
import { getQualAnalysisRequest, updateQualAnalysisRequest } from '@/lib/db';
...
export async function GET(
  ...
  try {
    ...
    const req = await getQualAnalysisRequest(id);
    if (!req) {
...
export async function PATCH(
  ...
  try {
    ...
    const before = await getQualAnalysisRequest(id);
    const updated = await updateQualAnalysisRequest(id, body);
    if (!updated) {
      return Response.json({ error: '의뢰를 찾을 수 없습니다.' }, { status: 404 });
    }
    if (before && body.status && before.status !== updated.status && updated.email) {
      await notifyStatusChanged(updated.email, '질적분석', updated.status);
    }
